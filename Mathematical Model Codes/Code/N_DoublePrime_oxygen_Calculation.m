function N_DoublePrime_oxygen=N_DoublePrime_oxygen_Calculation(f,p,e,theta)
    format long g
    A=[50.474214 0.975 9.651 6.690 0.0 2.566 6.850;
    ...
    S = zeros(1, numRows); % Row vector with 'numRows' columns
    F = zeros(1, numRows); % Row vector with 'numRows' columns
    ...
    for i=1:numRows
        % finding Si
        S(i)=A(i,2)*(10^-7)*p*(theta^3)*exp(A(i,3)*(1-theta));
        % finding Fi
        del=(A(i,6)+theta*A(i,7))*(10^-4)*(p+e)*(theta^0.8);
        del_f=A(i,4)*(10^-4)*(1.1*e*theta + p*(theta^(0.8-A(i,5)))     );
        del_f=sqrt((del_f)^2 + 2.25*(10^-6)); %considering zeeman splitting

        alu=(del_f-del*(A(i,1)-f))/(del_f^2 +(A(i,1)-f)^2);
        kochu=(del_f-del*(A(i,1)+f))/(del_f^2 +(A(i,1)+f)^2);

        F(i)=(f/A(i,1))*(alu+kochu);

        mango=mango+ S(i)*F(i);

    end

    N_DoublePrime_oxygen=mango+N_DoublePrime_D_oxygen_Calculation(f,p,e,theta);
    %N_DoublePrime_oxygen=0.012*N_DoublePrime_oxygen;
