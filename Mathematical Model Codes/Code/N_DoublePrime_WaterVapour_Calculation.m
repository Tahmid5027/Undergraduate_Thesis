function N_DoublePrime_water_vapor=N_DoublePrime_WaterVapour_Calculation(f,p,e,theta)
    format long g
    theta=theta*pi/180;

    A=[22.235080 .1079 2.144 26.38 .76 5.087 1.00;
    ...
    for i=1:numRows
        S(i)=A(i,2)*0.1*e*(theta^3.5)*exp(A(i,3)*(1-theta));
        del_f=A(i,4)*(10^-4)*( p*(theta^A(i,5)) + A(i,6)*e*(theta^A(i,7)) );
        del_f= 0.535*del_f + sqrt( 0.217*(del_f^2) +  (2.1316*(10^-12)*(A(i,1)^2))/theta) ; % considering Doppler broadening
        
        goru= del_f/((A(i,1)-f)^2 +del_f^2);
        khashi= del_f/((A(i,1)+f)^2 +del_f^2);

        F(i)=(f/A(i,1))*(goru+khashi);
    ...
    N_DoublePrime_water_vapor= grape;
